const pokerRaw = `8C TS KC 9H 4S 7D 2S 5D 3S AC
...
class Card {
  constructor(card) {
    this.suit = this.getSuit(card[1])
    this.number = this.getNumber(card[0])
  }
...
class Hand {
  ...
  // テストしてない。
  // メモ：Straight は Ace の扱いがややこしいかも。
  highCard() {
    this.hand = 0
    for (var i = 0; i < this.cards.length; ++i) {
      this.rank.push(this.cards[i].number)
    }
  }
    ...
    for (var i = 0; i <= 3; ++i) {
      ...
      for (var j = 0; j < this.cards.length; ++j) {
        if (this.cards[i].number === this.cards[j].number) {
          ...
          rank.push(this.cards[j].number)
        }
      }
      if (sum === 2) {
        this.hand = 1
        this.rank = [this.cards[i].number].concat(rank)
        return
      }
    }
    this.highCard()
  ...
  hasTwoPairs() {
    ...
    for (var i = 0; i < this.cards.length; ++i) {
      ...
      if (next === this.cards.length) {
        lowRank = this.cards[i].number
        break
      }
      if (this.cards[i].number === this.cards[next].number) {
        ++pair
        ++i
        rank.push(this.cards[i].number)
      }
      else {
        lowRank = this.cards[i].number
      }
  ...
  hasStraight() {
    for (var i = 1; i < this.cards.length; ++i) {
      if (this.cards[0].number - i !== this.cards[i].number) {
    ...
    this.hand = 4
    this.rank.push(this.cards[0].number)
  }
  ...
  hasFullHouse() {
    ...
    var lowRank = 0
    var highRank = 0
    if (this.cards[2].number === this.cards[1].number) {
      if (this.cards[2].number === this.cards[0].number
        && this.cards[3].number === this.cards[4].number) {
        isFullHouse = true
        lowRank = this.cards[4].number
        highRank = this.cards[0].number
      }
    }
    else {
      if (this.cards[2].number === this.cards[3].number
        && this.cards[2].number === this.cards[4].number
        && this.cards[0].number === this.cards[1].number) {
        isFullHouse = true
        lowRank = this.cards[0].number
        highRank = this.cards[4].number
      }
    }

    if (isFullHouse) {
      this.hand = 6
      this.rank.push(highRank, lowRank)
    }
    else {
      this.hasFlush()
    }
  ...
  hasFourOfAKind() {
    for (var i = 0; i < 2; ++i) {
      ...
      if (sum === 4) {
        this.hand = 7
        this.rank.push(this.cards[i].number, this.cards[nextHighest].number)
        return
      }
    }
    this.hasFullHouse()
  ...
  hasStraightFlush() {
    for (var i = 1; i < this.cards.length; ++i) {
      if (this.cards[0].suit !== this.cards[i].suit
        || this.cards[0].number - i !== this.cards[i].number) {
        this.hasFourOfAKind()
        return
      }
    }
  ...
  hasRoyalFlush() {
    if (this.cards[0].number !== 14 || this.cards[1].number !== 13
      || this.cards[2].number !== 12 || this.cards[3].number !== 11
      || this.cards[4].number !== 10) {
...
function compareHands(hand1, hand2) {
  ...
  for (var i = 0; i < hand1.rank.length; ++i) {
    if (hand1.rank[i] > hand2.rank[i]) {
      return 1
    }
    else if (hand1.rank[i] < hand2.rank[i]) {
      return 2
    }
  }
  return 0
...
var poker = pokerRaw.split(/\n/g)
for (var i = 0; i < poker.length; ++i) {
  poker[i] = poker[i].split(/ /g)
}
console.log(poker)
...
var sum = 0
for (var i = 0; i < poker.length; ++i) {
  var h1 = new Hand(poker[i].slice(0, 5))
  var h2 = new Hand(poker[i].slice(5, 10))
  if (compareHands(h1, h2) === 1) {
    ++sum
  }
}
console.log(sum)
...
// Royal Flush のテスト
// var h1cards = "TH JH QH KH AH".split(/ /g)
// var h2cards = "3C 3D 3S 9S 9D".split(/ /g)

// Straight Flush のテスト
// var h1cards = "2H 3H 4H 5H 6H".split(/ /g)
// var h2cards = "5D 6D 7D 8D 9D".split(/ /g)

// Four of a Kind のテスト
// var h1cards = "2H 2S 2D 2C 8D".split(/ /g)
// var h2cards = "9S 9D 9C 9H 8S".split(/ /g)

// Straight のテスト
// var h1cards = "2H 3S 4D 5C 6D".split(/ /g)
// var h2cards = "4S 5D 6C 7H 8S".split(/ /g)

// Three of a Kind のテスト
// var h1cards = "3H 3S 3D 5C 6D".split(/ /g)
// var h2cards = "3S 3D 3C 7H 8S".split(/ /g)

// Two Pairs のテスト
// var h1cards = "3H 3S 5D 7C 7D".split(/ /g)
// var h2cards = "3S 3C 5C 5H 8S".split(/ /g)

// var h1 = new Hand(h1cards)
// var h2 = new Hand(h2cards)
// console.log(h1, h2, compareHands(h1, h2))